...
/**
 * Convenient method for variable-length hashing using Shake256
 */
static void hash_with_shake256(uint8_t *out, size_t outlen,
                               keccak_state *hasher, const uint8_t *in,
                               size_t inlen) {
...
void etmkem_keypair(uint8_t *pk, uint8_t *sk) {
  ...
  keccak_state hasher;
  hash_with_shake256(sk + PKE_SECRETKEYBYTES, PKE_SYMBYTES, &hasher, pk,
                     PKE_PUBLICKEYBYTES);
...
void etmkem_encap(uint8_t *ct, uint8_t *ss, const uint8_t *pk) {
  ...
  hash_with_shake256(pkhash_pt, PKE_SYMBYTES, &hasher, pk, PKE_PUBLICKEYBYTES);
  ...
  hash_with_shake256(prekey_mackey_maciv,
                     ETMKEM_SSBYTES + MAC_KEYBYTES + MAC_IVBYTES, &hasher,
  ...
  memcpy(prekey_mactag + ETMKEM_SSBYTES, ct + PKE_CIPHERTEXTBYTES,
         MAC_TAGBYTES);
  hash_with_shake256(ss, ETMKEM_SSBYTES, &hasher, prekey_mactag,
...
void etmkem_decap(uint8_t *ss, const uint8_t *ct, const uint8_t *sk) {
  ...
  memcpy(prekey_mactag + ETMKEM_SSBYTES, ct + PKE_CIPHERTEXTBYTES,
         MAC_TAGBYTES);
  hash_with_shake256(true_ss, ETMKEM_SSBYTES, &hasher, prekey_mactag,
                     ETMKEM_SSBYTES + MAC_TAGBYTES);
  ...
  hash_with_shake256(false_ss, ETMKEM_SSBYTES, &hasher, rejection_ct,
