...
\textit{Remark} In our implementation, we used {\tt OpenSSL 3.3.1}'s \texttt{EVP\_MAC} interface. Within this interface, the Poly1305 implementation does not include a nonce and is thus only one-time secure. On the other hand, GMAC is implemented by passing all data into the associated data field of the authenticated encryption scheme AES-256-GCM. Assuming that nonce does not repeat within the lifetime of the symmetric key, GMAC is many-time secure.
...
\begin{figure}[h]
    ...
    \begin{minipage}[t]{0.5\textwidth}
        \begin{algorithm}[H]
            \caption*{$\texttt{ML-KEM-EtM}\texttt{.}\keygen\texttt{()}$}
            \begin{algorithmic}[1]
        ...
        \begin{algorithm}[H]
            \caption*{$\texttt{ML-KEM-EtM}\texttt{.}\encap\texttt{(\pk)}$}
            \begin{algorithmic}[1]
    ...
    \begin{minipage}[t]{0.49\textwidth}
        \begin{algorithm}[H]
            \caption*{$\texttt{ML-KEM-EtM}\texttt{.}\decap\texttt{(\sk, c)}$}
            \begin{algorithmic}[1]
    ...
    \caption{\texttt{ML-KEM-EtM} applies the encrypt-then-MAC KEM transformation to the K-PKE sub-routines of ML-KEM. $G$ is SHA3-512, $H$ is SHA3-256, $\texttt{KDF}$ is SHAKE256}\label{fig:ml-kem-plus-routines}
\end{figure}

We measured the CPU cycles of the individual routines of \texttt{ML-KEM-EtM}. Our implementation extended from Kyber's reference implementation \footnote{\url{https://github.com/pq-crystals/kyber}}. All C code is compiled with \texttt{GCC 11.4.1} and \texttt{OpenSSL 3.0.8}. All binaries are executed on an AWS c7a.medium instance (AMD EPYC 9R14 CPU at 3.7 GHz and 1 GB of RAM) in the \texttt{us-west-2} region.
...
\begin{table}[hp]
    ...
    %     \hline
    % \end{tabular}
    \begin{tabular}[t]{|p{15em}|p{4em}|p{4em}|p{4em}|p{4em}|}
        \hline
        ...
        % \hline
        % ML-KEM+-512
        \texttt{ML-KEM-EtM-512} + Poly1305 & 93157 & 93626 & 33733 & 33908 \\
        \hline
        \texttt{ML-KEM-EtM-512} + GMAC & 97369 & 97766 & 37725 & 37831 \\
        \hline
        \texttt{ML-KEM-EtM-512} + CMAC & 99739 & 99959 & 40117 & 39943 \\
        \hline
        \texttt{ML-KEM-EtM-512} + KMAC256 & 101009 & 101313 & 40741 & 40916 \\
        \hline
    ...
    \begin{tabular}[t]{|p{15em}|p{4em}|p{4em}|p{4em}|p{4em}|}
        ...
        \hline
        \texttt{ML-KEM-EtM-768} + Poly1305 & 146405 & 146860 & 43315 & 43463 \\
        \hline
        \texttt{ML-KEM-EtM-768} + GMAC & 149525 & 150128 & 46513 & 46706 \\
        \hline
        \texttt{ML-KEM-EtM-768} + CMAC & 153139 & 153735 & 49841 & 50074 \\
        \hline
        \texttt{ML-KEM-EtM-768} + KMAC256 & 155219 & 155848 & 52415 & 52611 \\
        \hline
    \end{tabular}\vspace{0.3cm}
    ...
    \begin{tabular}[t]{|p{15em}|p{4em}|p{4em}|p{4em}|p{4em}|}
        ...
        \hline
        \texttt{ML-KEM-EtM-1024} + Poly1305 & 205763 & 206499 & 51375 & 51562 \\
        \hline
        \texttt{ML-KEM-EtM-1024} + GMAC & 208805 & 209681 & 54573 & 54780 \\
        \hline
        \texttt{ML-KEM-EtM-1024} + CMAC & 213667 & 214483 & 59175 & 59408 \\
        \hline
        \texttt{ML-KEM-EtM-1024} + KMAC256 & 216761 & 217468 & 62269 & 62516 \\
        \hline
    \end{tabular}
\end{table}
\end{document}
